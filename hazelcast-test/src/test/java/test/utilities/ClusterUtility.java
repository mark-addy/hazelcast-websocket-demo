...
public class ClusterUtility {
	...
	public void populateClusterDataset(String mapName, int size, int backupCount, long delay) {
        buildAndApplyMapConfig(mapName, backupCount);
        Map<Integer, String> map = getRandomInstance().getMap(mapName);
        for (int i = 0 ; i < size ; i++) {
        	map.put(new Integer(i), "Value : " + i);
        }
        if (delay > 0) {
	        try {
	...
	public void populateClusterDataset(String mapName, int size, int backupCount) {
		populateClusterDataset(mapName, size, backupCount, 0L);
	}
